...
export function SetCard({ set }: SetCardProps) {
  const displayName = set.name_pt && set.name_pt.length > 0 ? set.name_pt : set.name
  const displaySeries = set.series_pt && set.series_pt.length > 0 ? set.series_pt : set.series
  ...
  return (
    ...
    >
      <div className="flex items-center justify-center h-16 mb-3">
        {set.image_url ? (
          // Using plain img because pokemontcg.io domain may not be in next.config
